...
21. **Execution RBAC materialization**: When the approved remediation option includes RBAC requests, before execution the controller MUST create a **per-proposal ServiceAccount** named `ls-exec-{proposal-name}` in the operator namespace (truncated to 63 chars), then create namespace-scoped `Role`+`RoleBinding` pairs in each target namespace and `ClusterRole`+`ClusterRoleBinding` for cluster-scoped rules, binding subjects to **this per-proposal SA** (not the shared `lightspeed-agent` SA). The per-proposal SA MUST carry an owner reference to the Proposal CR. Idempotent create MUST tolerate existing objects. See `agentic-security.md` rules 7-15 for full specification.
...
24. **RBAC cleanup**: When the proposal reaches configured terminal outcomes, fails fatally, completes escalation successfully, or is deleted, the controller MUST delete execution RBAC objects it created and the per-proposal ServiceAccount, using the annotation or equivalent persisted scope information. Owner references provide GC as a fallback, but explicit cleanup ensures prompt credential removal.
