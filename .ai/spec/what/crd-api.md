...
19. **Agent — `status.conditions` / `status.ready`**: Observed readiness. The operator MUST set `Ready` True when the referenced `LLMProvider` exists and its credentials Secret is present (with required keys) in the operator namespace, and False with a reason/message naming the missing object otherwise. `status.ready` MUST copy the Ready condition status (`True`/`False`) so `oc get agents` printer columns work (kube-apiserver does not evaluate JSONPath filters). [OLS-4224]
