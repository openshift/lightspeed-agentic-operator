...
20. **LLMProvider — discriminator**: `spec.type` MUST be one of the case-sensitive values `Anthropic`, `GoogleCloudVertex`, `OpenAI`, `AzureOpenAI`, or `AWSBedrock` and MUST match exactly one embedded config: `spec.anthropic`, `spec.googleCloudVertex`, `spec.openAI`, `spec.azureOpenAI`, or `spec.awsBedrock`; CEL enforces mutual exclusion.
20a. [PLANNED: OLS-3472] **Gemma 4 through vLLM**: A Gemma 4 model served by RHOAI/vLLM MUST use the existing `OpenAI` discriminator and `spec.openAI` configuration with the cluster-internal OpenAI-compatible URL. The referenced `Agent.spec.model` MUST equal the model identifier exposed by vLLM. No Gemma- or RHOAI-specific discriminator or embedded provider config is added.
...
- [PLANNED: OLS-3472] Verify the existing `openAI` provider API with Gemma 4 on an internal RHOAI/vLLM endpoint; no CRD shape change. See `../how/disconnected-product-e2e.md`.
